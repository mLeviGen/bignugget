...
def set_keyboard(user_id):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔄 Оновити", callback_data=f"refresh_map_{user_id}"
                )
            ],
            [
                InlineKeyboardButton(
...
async def handle_show_map(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        photo = FSInputFile(MAP)
        media = InputMediaPhoto(
            media=photo,
            caption=f"Мапа тривог\n\nДля користувача: {username}",
        )
        await callback_query.message.edit_media(
            media, reply_markup=set_keyboard(user_id)
        )
